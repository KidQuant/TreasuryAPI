...
class FederalTreasurySession:
    ...
    def __repr__(self) -> str:
        ...
        str_representation = (
            "<FederalTreasuryClient.TreasurySession (active=True, connected=True)>"
        )
    ...
    def make_request(
        ...
        if "realtime_start" in params and isinstance(
            params["realtime_start"], datetime
        ):
        ...
        if "exclude_tag_names" in params and isinstance(
            params["exclude_tag_names"], list
        ):
            logging.info(
                "Joining Exclude Tag Names: {lst}".format(
        ...
        logging.info("PARAMS: {params}".format(params=params_cleaned))
        ...
        request_request = requests.Request(
            method=method.upper(), url=url, params=params, data=data, json=json_payload
        ).prepare()
